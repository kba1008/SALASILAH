...
const HANDLERS = {
  ...
  setPositions(body) {
    ...
    list.forEach(function(p){
      if (!p || !p.id) return;
      const patch = {};
      if (p.x !== undefined) patch.posX = Number(p.x) || 0;
  ...
  updateMyProfile(body) {
    const u = requireAuth(body);
    const patch = {};
    ['fullName','fatherName','motherName'].forEach(function(k){
      if (body[k] !== undefined) patch[k] = upperName(body[k]).slice(0,200);
    });
    ['address','whatsapp','occupation','email','phone'].forEach(function(k){
    ...
    if (body.newPassword) {
      const salt = randomToken().slice(0,16);
      patch.salt = salt;
      patch.passwordHash = sha256Hex(String(body.newPassword)+salt);
